...
def analys_vaxe(files: list, idx=0, resultdir='minute', filter_stocks=[], bar_type=1):
    ...
    try:
        ...
        for file in files:
            ...
            # date = int(datetime.strptime(str(sorted(d.date.unique())[-1]), "%Y%m%d").timestamp())
            # dates[d.stock.unique()[0]] = date  # 记录股票对应的日期
            # d['time'] = date + d['time'].str.zfill(9)
            df.append(d)
        df = vaex.concat(df)
        ...
        for date in df.date.unique():
            full_time_df += [pd.date_range(start=f'{date} 01:30:01', end=f'{date} 03:30:00', freq=f'{bar_type}S'),
                             pd.date_range(start=f'{date} 05:00:01', end=f'{date} 07:00:00', freq=f'{bar_type}S')]
...
def statistic_trade(start_date, end_date, resultdir, rootdirs, filter_stock=[]):
    ...
    files = sorted(files, reverse=True, key=lambda x: os.path.basename(os.path.dirname(x)))
    files = list(map(lambda x: list(x[1]), groupby(files, key=lambda x: os.path.basename(os.path.dirname(x)))))
    logger.info(f'files count: {len(files)}, filter_stock: {filter_stock}')
    with ProcessPoolExecutor(max_workers=18) as executor:
        os.system(f'rm {resultdir} -rf && mkdir {resultdir}')
        futures = [executor.submit(analys_vaxe, arg, idx=idx, resultdir=resultdir, filter_stocks=filter_stock) for idx, arg in enumerate(files)]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--rootdir', type=str, default='/data/share/w2024', help='数据根目录，默认/data/transaction/20231226', required=False)
    ...
    statistic_trade(start_date, end_date, args.resultdir, args.rootdir.split(','))
