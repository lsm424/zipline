...
def handle_data(context, data):
    ...
    logger.info(f"[{context.i}]{data.current_dt}, real_time: {real_time}, 耗时：{round(time.time() - start, 3)}s, 耗时1：{round(time_stampe1 - start, 3)}  耗时2：{round(time_stampe2 - time_stampe1, 3)}s ，剩余cash:{context.portfolio.cash}")
